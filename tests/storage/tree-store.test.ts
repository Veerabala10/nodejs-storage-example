...
describe(`full test of the SMT library: mongodb`, () => {
  ...
  it('checks that the implementation of the db.Storage interface behaves as expected', async () => {
    const sto = await getTreeStorage();
  ...
  it('test new merkle tree', async () => {
    ...
    expect((await sto.getRoot()).bigInt().toString()).equal((await mt.root()).bigInt().toString());
  ...
  it('test add and different order', async () => {
    const sto1 = await getTreeStorage('tree1', 'mt1');
    const sto2 = await getTreeStorage('tree2', 'mt2');
  ...
  it('test update 2', async () => {
    const sto1 = await getTreeStorage('tree1');
    const sto2 = await getTreeStorage('tree2');
    const mt1 = new Merkletree(sto1, true, 140);
    const mt2 = new Merkletree(sto2, true, 140);
